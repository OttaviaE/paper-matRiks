...
author:  
  - name: Ottavia M. Epifania
    affiliation: Univerisity of Trento
    address:
    - Department of Psyhcology and Social Science
    - Rovereto, Italy
...
@plotly  cattell1963 defined fluid intelligence (*g*) as the ability of solving novel reasoning problems that has little to do with concepts learned in schools or through acculturational processes.
...
indicating that the square is a single figure, it can be filled with different shadings, and it can be rotated. The tag `d.ext` specifically refers to the possibility of the figure to be used for the generation of the Difference distractor. Further details are provided in Section XX. 
The tags associated to malta cross are: 

```{r echo=FALSE}
malta()$tag
```
which indicate that the figure is composed of 4 different figuers, it can be filled with different shading but it cannot rotate (it cn be inferred from the fact that the `rotate` tag is missing).
...
The categories
of the figures available in `matRiks`, along with their associated vignettes, are illustrated in  Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:figures-list-interactive)', '\\@ref(tab:figures-list-static)'))`.
...
```{r figures-list-static, eval = knitr::is_latex_output()}
tbl_img = data.table(
...
```{r rule-types-static, eval = knitr::is_latex_output(), layout = "l-body-outset"}
